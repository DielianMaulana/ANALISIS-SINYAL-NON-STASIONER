...
def main():
    ...
    if uploaded_file is not None:
        ...
        with tab1:
            ...
            ax[0].plot(waktu, x, color='blue')
            ax[0].set_title('ECG Signal')
            ax[1].plot(waktu, resp, color='pink')
            ax[1].set_title('Respiratory Signal')
            plt.tight_layout()
            st.pyplot(fig)
        ...
        with tab2:
            ...
            fig, axes = plt.subplots(nrows=8, ncols=1, figsize=(20, 40), sharex=True)

            for i in range(8):
                axes[i].plot(waktu, signals[i], color=colors[i])
                axes[i].set_ylabel(f'D{i+1}')
                axes[i].set_xlabel('Time (s)')
                axes[i].set_title(f'DWT Level-{i+1}')
        ...
        with tab3:
            ...
            def absolute(signal):
                absolute_signal = np.zeros(np.size(signal))
                for i in range(len(signal)):
                    if signal[i] < 0:
                        absolute_signal[i] = -signal[i]
                    else:
                        absolute_signal[i] = signal[i]
                return absolute_signal

            abs_signal  = absolute(detail_coeffs_1)
            abs_signal2 = absolute(detail_coeffs_2)
            abs_signal3 = absolute(detail_coeffs_3)

            signals_sq = [abs_signal, abs_signal2, abs_signal3]
            colors = ['salmon', 'orange', 'green']

            fig1, axes1 = plt.subplots(nrows=3, ncols=1, figsize=(20, 6), sharex=True)
            for i in range(3):
                axes1[i].plot(waktu, signals_sq[i], color=colors[i])
                axes1[i].set_ylabel(f'D{i+1}')
                axes1[i].set_xlabel('Time (s)')
                axes1[i].set_title(f'Squaring ECG Signal Level-{i+1}')
            ...
            # Gunakan parameter MAV yang berbeda untuk tiap sinyal (sesuai kebutuhan)
            mav_signal  = moving_average(abs_signal, 10)
            mav_signal2 = moving_average(abs_signal2, 5)
            mav_signal3 = moving_average(abs_signal3, 5)

            signals_mav = [mav_signal, mav_signal2, mav_signal3]

            fig2, axes2 = plt.subplots(nrows=3, ncols=1, figsize=(20, 6), sharex=True)
            for i in range(3):
                axes2[i].plot(waktu, signals_mav[i], color=colors[i])
                axes2[i].set_ylabel(f'D{i+1}')
                axes2[i].set_xlabel('Time (s)')
                axes2[i].set_title(f'MAV ECG Signal Level-{i+1}')
            ...
            threshold_signal  = threshold(mav_signal,  0.1, 1)
            threshold_signal2 = threshold(mav_signal2, 0.25, 1)
            threshold_signal3 = threshold(mav_signal3, 0.45, 1)

            signals_thresh = [threshold_signal, threshold_signal2, threshold_signal3]

            fig3, axes3 = plt.subplots(nrows=3, ncols=1, figsize=(20, 6), sharex=True)
            for i in range(3):
                axes3[i].plot(waktu, signals_thresh[i], color=colors[i])
                axes3[i].set_ylabel(f'D{i+1}')
                axes3[i].set_xlabel('Time (s)')
                axes3[i].set_title(f'Thresholding ECG Signal Level-{i+1}')
        ...
        with tab4:
            ...
            # Pastikan 'signals' sudah berisi sinyal thresholding (misalnya dari tahap sebelumnya)
            for i, signal in enumerate(signals_thresh):
                # Deteksi puncak R dengan jarak minimal antar puncak (0.3 detik)
                peaks, _ = scipy.signal.find_peaks(signal, distance=fs*0.3)
                all_peaks.append(peaks)

                # Hitung interval antar puncak (RR intervals) dalam detik
                rr_intervals = np.diff(peaks) / fs

                # Hitung BPM per interval: BPM = 60 / RR interval
                bpm_list = []
                if len(rr_intervals) > 0:
                    for rr in rr_intervals:
                        bpm_list.append(60 / rr)
                    all_bpm.append(bpm_list)
                else:
                    all_bpm.append([])  # Jika tidak ada puncak R terdeteksi

            # --- Tampilkan hasil deteksi BPM untuk setiap sinyal ---
            st.subheader("Hasil BPM")
            for i, bpm_list in enumerate(all_bpm):
                if bpm_list:
                    st.write(f"**Sinyal D{i+1}:**")
                    avg_bpm = np.mean(bpm_list)
                    st.write(f"Rata-rata BPM: {avg_bpm:.2f}")
                    df = pd.DataFrame({
                        'Beat Number': range(1, len(bpm_list)+1),
                        'BPM': [f"{x:.2f}" for x in bpm_list]
                    })
                    st.table(df)
                else:
                    st.write(f"Tidak cukup puncak R terdeteksi untuk sinyal D{i+1}.")

            # --- Plot hasil deteksi puncak untuk setiap sinyal ---
            fig = plt.figure(figsize=(20, 10))
            colors = ['salmon', 'orange', 'green']
            
            for i, signal in enumerate(signals_thresh):
                ax = fig.add_subplot(len(signals), 1, i + 1)
                ax.plot(signal, color='blue', label='Threshold Signal')
                # Tampilkan puncak R yang terdeteksi
                if i < len(all_peaks):
                    peaks = all_peaks[i]
                    ax.plot(peaks, [signal[p] for p in peaks], 'ro', label='Detected R-peaks')
                
                ax.set_title(f'Detected R-peaks from Threshold Signal D{i+1}')
        ...
        with tab5:
            st.header("Perbandingan Sinyal Pernapasan")
            
            fig, ax = plt.subplots(2, 1, figsize=(12, 4))
            ax[0].plot(waktu, resp, color='pink')
            ax[0].set_title('Respiratory Signal')
            
            ax[1].plot(waktu, detail_coeffs_7, color='blue')
            ax[1].set_title('Processed ECG Signal')
